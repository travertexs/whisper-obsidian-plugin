...
var Controls = class extends import_obsidian.Modal {
  ...
  onOpen() {
    ...
    dropdown.value = ((_a = commonLanguages.find(
      (lang) => lang.value === this.plugin.settings.transcriptionSTTLanguage
...
var OPENAI_API_URI = "https://api.openai.com/v1";
var GEMINI_API_URI = "https://generativelanguage.googleapis.com/v1beta";
var ANTHROPIC_API_URI = "https://api.anthropic.com/v1";
...
var AudioHandler = class {
  ...
  async sendAudioData(blob, fileName) {
    ...
    let audioFilePath = `${this.plugin.settings.audioSavingPath ? `${this.plugin.settings.audioSavingPath}/` : ""}${timestampedFileName}`;
    ...
    try {
      ...
      if (this.plugin.settings.transcriptionMode == "llm-mode" && this.plugin.settings.transcriptionLLMModel && this.plugin.settings.transcriptionLLMPrompt) {
        if (this.plugin.settings.transcriptionLLMProvider === "openai") {
          transcriptionResponse = await axios_default.post(
              ...
              messages: [
                ...
                },
                {
                  ...
                  content: [{
                    ...
                    input_audio: {
                      data: blob,
                      format: this.plugin.settings.silenceRemovalToggle ? "wav" : "mp3"
                    }
                  }]
                }
              ],
          ...
          transcriptionResponse = await axios_default.post(
            GEMINI_API_URI + "/models/" + this.plugin.settings.transcriptionLLMModel + ":generateContent?key=" + this.plugin.settings.geminiAPIKey,
            {
              system_instruction: {
                parts: [
                  {
              ...
              contents: [
                {
                  ...
                  parts: [
                    {
                      inlineData: {
                        mimeType: this.plugin.settings.silenceRemovalToggle ? "audio/wav" : "audio/mp3",
                        data: base64
                      }
                    }
                  ]
        ...
        transcriptionResponse = await axios_default.post(
          (isProviderOpenAI ? OPENAI_API_URI : this.plugin.settings.openAIFormatAPIUri) + "/audio/transcriptions",
          formData,
          {
            headers: {
              "Content-Type": "multipart/form-data",
              Authorization: isProviderOpenAI ? `Bearer ${this.plugin.settings.openAIAPIKey}` : `Bearer ${this.plugin.settings.openAIFormatAPIKey}`
            }
          }
        ).catch((error) => {
          if (error.response) {
      ...
      if (this.plugin.settings.postProcessingToggle && this.plugin.settings.postProcessingModel && this.plugin.settings.postProcessingPrompt && this.plugin.settings.transcriptionMode === "stt-mode") {
        if (this.plugin.settings.debugMode) {
        ...
        try {
          ...
          if (this.plugin.settings.postProcessingProvider === "openai") {
            ...
            postProcessResponse = await axios_default.post(
              GEMINI_API_URI + "/models/" + this.plugin.settings.postProcessingModel + ":generateContent?key=" + this.plugin.settings.geminiAPIKey,
              {
                system_instruction: {
                  parts: [
                    {
      ...
      let finalTitle = null;
      if (this.plugin.settings.titleGenerationToggle && this.plugin.settings.titleGenerationModel && this.plugin.settings.titleGenerationPrompt) {
        if (this.plugin.settings.debugMode) {
        ...
        try {
          ...
          if (finalTitle) {
            ...
            audioFilePath = `${this.plugin.settings.audioSavingPath ? `${this.plugin.settings.audioSavingPath}/` : ""}${nowFileName}`;
            ...
            if (this.plugin.settings.audioSavingToggle) {
              const oldAudioPath = `${this.plugin.settings.audioSavingPath ? `${this.plugin.settings.audioSavingPath}/` : ""}${timestampedFileName}`;
      ...
      let noteContent = this.plugin.settings.audioSavingToggle ? `![[${audioFilePath}]]
${finalText}` : `${finalText}`;
...
var WhisperSettingsTab = class extends import_obsidian3.PluginSettingTab {
  ...
  createAPISettings() {
    ...
    new import_obsidian3.Setting(this.containerEl).setName("Choose platform settings").setDesc("Select which platform settings to display.").addDropdown((dropdown) => {
    ...
    if (this.plugin.settings.apiProviderSetting === "openai") {
      new import_obsidian3.Setting(this.containerEl).setName("OpenAI API Key").setDesc("Enter your OpenAI API key to use models from OpenAI.").addText((text) => {
      ...
      new import_obsidian3.Setting(this.containerEl).setName("OpenAI Format API Key").setDesc("Enter your Third-party API key to use custom models").addText((text) => {
      ...
      new import_obsidian3.Setting(this.containerEl).setName("OpenAI Format API URI").setDesc("Specify the endpoint that will be used to make requests to.").addText((text) => {
      ...
      new import_obsidian3.Setting(this.containerEl).setName("Gemini API Key").setDesc("Enter your Gemini API key for Gemini models").addText((text) => {
      ...
      new import_obsidian3.Setting(this.containerEl).setName("Anthropic API Key").setDesc("Enter your Anthropic API key for Claude models").addText((text) => {
  ...
  createTranscriptionSettings() {
    ...
    new import_obsidian3.Setting(this.containerEl).setName("Transcription Mode").setDesc("Select which transcription mode to use for transcribing.").addDropdown((dropdown) => {
    ...
    if (this.plugin.settings.transcriptionMode === "llm-mode") {
      ...
      new import_obsidian3.Setting(this.containerEl).setName("Choose provider for LLM").setDesc("Select which provider to use.").addDropdown((dropdown) => {
        for (const [key, value] of Object.entries(providers)) {
          dropdown.addOption(key, value);
        }
      ...
      new import_obsidian3.Setting(this.containerEl).setName("LLM").setDesc("Specify the LLM to use for transcribing audio to text (gpt-4o-mini-audio for OpenAI, gemini-2.0-flash for Gemini, etc)").addText((text) => {
      ...
      new import_obsidian3.Setting(this.containerEl).setName("Temperature for transcribing").setDesc("Specify the temperature of the LLM. A value between 0 and 2. Higher values (closer to 2) means more creative outputs.").addSlider((slider) => {
      ...
      new import_obsidian3.Setting(this.containerEl).setName("LLM Prompt").setDesc("Enter the prompt that will be sent to the LLM to transcribe the audio.").addTextArea((textArea) => {
      ...
      new import_obsidian3.Setting(this.containerEl).setName("Choose provider for speech-to-text model").setDesc("Select which provider to use.").addDropdown((dropdown) => {
        for (const [key, value] of Object.entries(providers)) {
          dropdown.addOption(key, value);
        }
      ...
      new import_obsidian3.Setting(this.containerEl).setName("Speech-to-text Model").setDesc("Specify the speech-to-text model to use for transcribing audio to text (whisper-1 for OpenAI, whisper-large-v3 for Groq, etc)").addText((text) => {
      ...
      new import_obsidian3.Setting(this.containerEl).setName("Language").setDesc("Specify the language of the speech (e.g. en for English). This can be left blank for auto-detection with some models, but some has to be set.").addText((text) => {
      ...
      new import_obsidian3.Setting(this.containerEl).setName("Speech-to-text Prompt").setDesc("Optional: Add words with their correct spellings to help with transcription. Make sure it matches the chosen language.").addTextArea((textArea) => {
  ...
  createPostProcessingSettings() {
    if (this.plugin.settings.transcriptionMode === "stt-mode") {
      new import_obsidian3.Setting(this.containerEl).setName("Use Post-processing").setDesc("Turn on to post-process the transcribed text using GPT.").addToggle((toggle) => {
      ...
      new import_obsidian3.Setting(this.containerEl).setName("Post-processing Model").setDesc("Specify the AI model to use for post-processing (gpt-4o-mini-audio for OpenAI, gemini-2.0-flash for Gemini, etc).").addText((text) => {
      ...
      new import_obsidian3.Setting(this.containerEl).setName("Temperature for post-processing").setDesc("Specify the temperature of the LLM. A value between 0 and 2. Higher values (closer to 2) means more creative outputs.").addSlider((slider) => {
        slider.setLimits(0, 2, 0.1);
      ...
      new import_obsidian3.Setting(this.containerEl).setName("Post-processing Prompt").setDesc("Enter the prompt that will be sent to the GPT model to polish the transcription.").addTextArea((textArea) => {
        textArea.setPlaceholder("Enter your prompt here...");
      ...
      new import_obsidian3.Setting(this.containerEl).setName("Keep Original Transcription").setDesc("If enabled, adds the original transcription below the post-processed text.").addToggle((toggle) => {
  ...
  createTitleGenerationSettings() {
    new import_obsidian3.Setting(this.containerEl).setName("Auto-generate Title").setDesc("Turn on to automatically generate a title for the transcribed text.").addToggle((toggle) => {
    ...
    new import_obsidian3.Setting(this.containerEl).setName("Title Generation Model").setDesc("Specify the AI model to use for post-processing (gpt-4o-mini-audio for OpenAI, gemini-2.0-flash for Gemini, etc).").addText((text) => {
    ...
    new import_obsidian3.Setting(this.containerEl).setName("Temperature for title generating").setDesc("Specify the temperature of the LLM. A value between 0 and 2. Higher values (closer to 2) means more creative outputs.").addSlider((slider) => {
      slider.setLimits(0, 2, 0.1);
    ...
    new import_obsidian3.Setting(this.containerEl).setName("Title-generation Prompt").setDesc("The prompt used to generate a title from the transcribed text.").addTextArea((textArea) => {
  ...
  createSilenceRemovalSettings() {
    ...
    new import_obsidian3.Setting(this.containerEl).setName("Remove Silence").setDesc("Remove silence from audio before processing (final file will be WAV).").addToggle((toggle) => {
    ...
    new import_obsidian3.Setting(this.containerEl).setName("Silence Threshold").setDesc("Sound level (in dB) below which audio is considered silence. Lower values are more aggressive (-50 is default)").addSlider((slider) => {
    ...
    new import_obsidian3.Setting(this.containerEl).setName("Minimum Silence Duration").setDesc("Minimum duration (in seconds) of silence to remove").addSlider((slider) => {
      slider.setLimits(0.05, 10, 0.1);
    ...
    new import_obsidian3.Setting(this.containerEl).setName("Remove All Silence").setDesc("When enabled, removes all silent periods throughout the audio. When disabled, only removes leading and trailing silence.").addToggle((toggle) => {
